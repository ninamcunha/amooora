...
with tab1:
    with st.form("my_form"):
        name = st.text_input('Name', value="Francis", placeholder='Your name here')
        ...
        orientation = st.selectbox('Orientation ', ['', 'Bisexual', 'Gay', 'Straight'], index=1)
        body_type = st.selectbox('Body type ', ['', 'Athletic', 'Average', 'Curvy', 'Fit', 'Thin', 'Other', 'Not Disclosed'], index=1)
        drinks = st.selectbox('Drinks 🍺🍸🍷', ['', 'Not at all', 'Often', 'Rarely', 'Socially'], index=1)
        diet = st.selectbox('Diet', ['', 'Anything', 'Other', 'Vegetarian'], index=1)
        height = st.number_input('Height', format="%.2f", value=1.78)
        have_kids = st.selectbox('Do you have 👶(s)?', ['', 'More than one', 'One', 'Zero'], index=1),
